  ...
  <div class="row">
        ...
        <div class="card">
          ...
          <div class="card-body">
            <form name = "facility_form">
              ...
              <div class="row mb-3">
                <label for="inputText" class="col-sm-3 col-form-label">Facility Name</label>
                <div class="col-sm-9">
                  <input type="text" class="form-control" name ="name" placeholder="(e.g Barangay Hall)">
                </div>
              </div>
            ...
            <div class="row mb-3">
              <div class="col-sm-12">
                <button type="submit" class="btn btn-primary" name = "create_facility_record">Submit Form</button>
              </div>
            </div>
    ...
    <div class="col-lg-7 col-sm-12">
      <div class="card-datatable table-responsive">
        <table class="table" name = "facility_record_table">
            <thead>
            <tr>
                <th>Baranggay</th>
                <th>Facility Name</th>
...
<script type="text/javascript">
  const facility_form =  $('form[name="facility_form"]');
  const remove_edit_button =  $('i[name="remove_edit_button"]');
  const card_title =  $('h5[name="card_title"]');

  const facility_record_table =  $('table[name="facility_record_table"]').DataTable({
      ajax: `/administrator/get_facility_record`,
      columns: [
          {data: 'baranggay_name'},
          {data: 'name'},
          {data: 'address'},
  ...
  $(document).on('click', 'table[name="facility_record_table"] tbody tr', function(){
    let data = facility_record_table.row(this).data();
    console.log(data);
    remove_edit_button.show();
    card_title.text("Modify Record");
    for(let key in data){
      facility_form.find(`input[name="${key}"]`).val(data[key]);
    }
  ...
  $('button[name="create_facility_record"]').on('click', function(){
    $.ajax({
      url: `/administrator/create_facility_record`,
      data: facility_form.serializeArray(),
      success: function(e){
        Toastify({
                  text: "Success",
                  duration: 3000,
